...
export default function Page() {
  ...
  const { fetchCourse, courseList, changeCourse, currentCourseIndex } =
    useCourse();
...
function Card({
  ...
  index: number;
  course: any;
  selectClick: (course: any, index: number) => void;
  currentCourseIndex: number;
}) {
  return (
    <>
      <div
        className={`w-52 h-24 bg-indigo-500 rounded-md p-1 shadow-lg hover:bg-indigo-700 cursor-pointer flex justify-center items-center transition-colors ${
          index === currentCourseIndex ? "bg-violet-700" : ""
        }`}
